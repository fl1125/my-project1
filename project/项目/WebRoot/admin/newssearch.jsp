            ...
            <div class="row-fluid">
				<div class="btn-toolbar">
					<div class="search-well">
						<form action="<%=path %>/admin/searchNews.do" method="post" class="form-inline">
							<input class="input-xlarge" placeholder="标题..." id="title" name="title" value="${title}" type="text">
    ...
    <table class="table">
      ...
      <c:forEach items="${list}" var="news">
        <tr>
          ...
          <td>${news.savetime}</td>
          <td>
              <a href="<%=path %>/admin/newsShow.do?id=${news.id}"><i class="icon-pencil"></i></a>
              <a href="<%=path %>/admin/newsDel.do?id=${news.id}" ><i class="icon-remove"></i></a>
          </td>
        </tr>
        </c:forEach>
        <tr >
         <td style="font-weight: bold;font-family:楷体;font-weight: bold; color:blue;text-align: right;" colspan="8">
                                                                 共${total}条记录&nbsp;&nbsp;&nbsp;&nbsp;
                        <a href="<%=path %>/admin/searchNews.do?index=1&title=${title}" style="font-family:微软雅黑;">首页</a>&nbsp;&nbsp;
                        
                        <c:choose>
                        <c:when test="${index >1}">
                        <a href="<%=path %>/admin/searchNews.do?index=${index-1}&title=${title}" style="font-family:微软雅黑;">上一页</a>
                        </c:when>
                        <c:otherwise>
                        ...
                        <a href="<%=path %>/admin/searchNews.do?index=${index+1}&title=${title}" style="font-family:微软雅黑;">下一页</a>
                        </c:when>
                        <c:otherwise>
                        ...
                        &nbsp;&nbsp;
                        <a href="<%=path %>/admin/searchNews.do?index=${pages}&title=${title}" style="font-family:微软雅黑;">末页</a>
                    </td>
       </tr>
      </tbody>
    </table>
