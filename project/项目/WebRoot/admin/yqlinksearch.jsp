            ...
            <div class="row-fluid">
				<div class="btn-toolbar">
					<div class="search-well">
						<form action="<%=path %>/admin/searchYqlink.do" method="post" class="form-inline">
...
<div class="well">
    <table class="table">
      ...
      <tbody>
      <c:forEach items="${list}" var="yqlink">
        <tr>
          <td>${yqlink.linkname }</td>
          <td>${yqlink.linkurl }</td>
          <td>
              <a href="<%=path %>/admin/yqlinkShow.do?id=${yqlink.id }"><i class="icon-pencil"></i></a>
              <a href="<%=path %>/admin/yqlinkDel.do?id=${yqlink.id }" ><i class="icon-remove"></i></a>
          </td>
        </tr>
      </c:forEach>
      <tr >
         <td style="font-weight: bold;font-family:楷体;font-weight: bold; color:blue;text-align: right;" colspan="8">
                                                                 共${total}条记录&nbsp;&nbsp;&nbsp;&nbsp;
                        <a href="<%=path %>/admin/searchYqlink.do?index=1&key=${key}" style="font-family:微软雅黑;">首页</a>&nbsp;&nbsp;
                        
                        <c:choose>
                        <c:when test="${index >1}">
                        <a href="<%=path %>/admin/searchYqlink.do?index=${index-1}&key=${key}" style="font-family:微软雅黑;">上一页</a>
                        </c:when>
                        <c:otherwise>
                        ...
                        <a href="<%=path %>/admin/searchYqlink.do?index=${index+1}&key=${key}" style="font-family:微软雅黑;">下一页</a>
                        </c:when>
                        <c:otherwise>
                        ...
                        &nbsp;&nbsp;
                        <a href="<%=path %>/admin/searchYqlink.do?index=${pages}&key=${key}" style="font-family:微软雅黑;">末页</a>
                    </td>
       </tr>
      </tbody>
    </table>
