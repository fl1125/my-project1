            ...
            <div class="row-fluid">
				<div class="btn-toolbar">
					<div class="search-well">
						<form action="admin/searchTzinfo.do" method="post" class="form-inline">
    ...
    <table class="table">
      ...
      <c:forEach items="${list}" var="tzinfo">
        ...
        <tr>
          ...
          <td><a href="tzDetail.do?id=${tzinfo.id}" target="_blank">${tzinfo.title}</a></td>
          <td>${tzinfo.isjh}</td>
          <td>${tzinfo.istop}</td>
          <td>${tzinfo.member.tname}</td>
          <td>${tzinfo.savetime}</td>
          <td>${tzinfo.looknum}</td>
          <td>${fn:length(tzinfo.allhtlist) }</td>
          <td>
              <a href="admin/findBbstype.do?id=${tzinfo.id}">迁移</a>
              <c:choose>
    ...
    <script type="text/javascript">
        ...
        $(function(){
        	  ...
        	  $('#key1').change(function(){
	        	  $.ajax({
		        	  url:"admin/searchBbstype.do?fid="+$('#key1').val(),
		        	  type:"post",
		        	  date:"",
        ...
        function tzdel(id){
					...
					$.ajax({  
				        ...
				        success: function(data){   
				        	if(data==0){
				        		alert('删除成功!');
				        		location.href="admin/tzinfoList.do";
				        	}
		...
		function tzzd(id){
					$.ajax({  
				        type: "POST",      
				        url: "admin/topTzinfo.do", //servlet的名字     
				        data: "id="+id, 
				        success: function(data){   
				        	alert('置顶成功!');
						    location.href="admin/tzinfoList.do";
...
function tzjj(id){
					$.ajax({  
				        type: "POST",      
				        url: "admin/jingHua.do", //servlet的名字     
				        data: "id="+id, 
				        success: function(data){   
				        	alert('加精成功!');
						    location.href="admin/tzinfoList.do";
...
function tzzdqx(id){
					$.ajax({  
				        type: "POST",      
				        url: "admin/notopTzinfo.do", //servlet的名字     
				        data: "id="+id, 
				        success: function(data){   
				        	alert('取消置顶成功!');
						    location.href="admin/tzinfoList.do";
...
function tzjjqx(id){
					$.ajax({  
				        type: "POST",      
				        url: "admin/noJinghua.do", //servlet的名字     
				        data: "id="+id, 
				        success: function(data){   
				        	alert('取消加精成功!');
						    location.href="admin/tzinfoList.do";
