      ...
      <div class="fly-panel">
        ...
        <c:forEach items="${list}" var="tzinfo">
          <li>
            ...
            <a href="tzDetail.do?id=${tzinfo.id}" class="jie-title">${tzinfo.title}</a>
            <i>${tzinfo.savetime}</i>
...
function qd(memberid){
	...
	$.ajax({  
        type: "POST",      
        url: "signinAdd.do", //servlet的名字     
        data: "memberid="+memberid, 
        success: function(data){   
        	if(data==0){
        		//layer.msg('关注成功!');
        		location.href="Home.do?memberid="+memberid;
        	}else{
        		layer.msg('你已关注过此人!');
        	}
...
function gz(memberid,gzmemberid){
					...
					$.ajax({  
				        type: "POST",      
				        url: "fansAdd.do", //servlet的名字     
				        data: "memberid="+memberid+"&gzmemberid="+gzmemberid, 
				        success: function(data){   
				        	if(data==0){
				        		layer.msg('关注成功!');
				        		location.href="Home.do?memberid="+gzmemberid;
				        	}else{
				        		layer.msg('你已关注过此人!');
				        	}
...
function pb(memberid,pbmemberid){
					...
					$.ajax({  
				        type: "POST",      
				        url: "pbinfoAdd.do", //servlet的名字     
				        data: "memberid="+memberid+"&pbmemberid="+pbmemberid, 
				        success: function(data){   
				        	layer.closeAll();
				        	if(data=='0'){
				        		layer.msg('屏蔽成功!');
				        		location.href="Home.do?memberid="+pbmemberid;
				        	}else{
				        		layer.msg('你已关注过此人!');
				        	}
...
function jb(jbmemberid){
	location.href="jbShow.do?jbmemberid="+jbmemberid;
}
function sx(memberid,sxmemberid){
	location.href="sxShow.do?memberid="+memberid+"&sxmemberid="+sxmemberid;
}
</script>
